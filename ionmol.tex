...
clásica~(\acs{cob}) ha sido utilizado para estudiar la ionización de 
agua, bases del ADN y ARN debido al impacto de protones y partículas 
...
geometría optimizada y expansión de un 
centro (\acs{sce})~\cite{DalCappello:08}, y de omisión completa de 
superposición diferencial (\acs{cndo})~\cite{Champion:10}. En este 
último trabajo, la hipótesis principal se basa en el modelo de átomo 
independiente~(\acs{iam}), donde las secciones eficaces moleculares se 
obtienen a partir de la combinación lineal de secciones eficaces 
atómicas con factores de peso moleculares. 
...
Miraglia:09}. Esta metodología también ha sido utilizada para modelar 
...
En este Capítulo se tratarán los dos aspectos principales de la 
...
En la Sección~\ref{sec:scaling}, se estudian diversas reglas de escala. 
...
\begin{comment}
La ecuación de Schr\"{o}dinger se resuelve usando un par de 
miles de puntos como pivotes, dependiendo del número de oscilaciones del 
estado del continuo. La integración radial fue realizada usando la 
técnica de interpolación cúbica. Las funciones de onda del estado final 
fueron expandidas como
\begin{equation}
...
para las energías más altas consideradas. Se requirieron el mismo número 
de ángulos azimutales para obtener las secciones eficaces diferenciales 
cuádruples. 
...
alrededor de 45 valores de energía electrónica, dependiendo de la 
energía de impacto del proyectil. Para más detalle sobre la metodología, 
se puede consultar el trabajo de la 
...
% Toburen, L. H. (1972). K-Shell Ionization of Carbon by Fast Protons. 
% Physical Review A, 5(6), 2482–2487. doi:10.1103/physreva.5.2482 
...
de impacto que se consideran van de $0.1$ a 10 MeV/amu, donde el método 
CDW tiene validez. Particularmente, para los proyectiles de carga más 
altas, el valor de energía mínimo donde se espera que la aproximación 
tenga validez aumenta hasta aproximadamente 400~keV. Los valores 
teóricos CDW se comparan con secciones eficaces experimentales 
disponibles en la literatura, tales como la ionización por impacto de 
H$^+$ en H~\cite{Shah:81}, C~\cite{Toburen:72}, N~\cite{Rudd:85} y 
...
%El rango de velocidad de impacto fue extiende sólo hasta $v=10$~a.u. debido 
%a las limitaciones numéricas en la expansión de esféricos armónicos 
%dados por la Ec.~(\ref{eq:contwave}). 
...
100--400 keV/amu. El efecto observado puede atribuirse a la repulsión 
electrónica causada por los iones de carga múltiple en los electrones 
de baja energía. Este comportamiento no se puede encontrar en la primera 
...
Para ilustrar esta fenómeno, se considera el impacto de C$^{6+}$ con una 
...
dentro del margen de error. Si bien esta data es preliminar (publicada 
recientemente en la tesis doctoral de Sens), la comparación es 
alentadora en cuanto a la respuesta del modelo para la ionización por 
...
mediciones de ionización por impacto de electrones (\acs{eie}) están 
por encima de nuestros hallazgos para todos los objetivos moleculares. 
Vale la pena señalar que los resultados SSM--CDW concuerdan muy bien con 
otras predicciones teóricas de EIE~\cite{mozejko2003,tan2018}. Más aún, 
recientes cálculos por Zein y colaboradores~\cite{Zein:21} usando el 
modelo de encuentro binario de Bethe (\acs{beb}) sostienen esta 
tendencia.
...
números de Toburen de los blancos atómicos que se consideran en este 
trabajo están dados por 
\begin{equation}
\nu_{\alpha}^T=\left\{ 
\begin{array}{ll}
1, & \text{para H,} \\
...
de la capa K es mayor. Así, en esta región, el número total de 
electrones activos será diferente. Una dependencia similar con el número 
de electrones débilmente ligados fue hallada por Itoh y 
colaboradores~\cite{itoh2013} para el impacto de protones sobre uracilo 
y adenina.

Siguiendo la ley de escala de Toburen, las secciones eficaces CDW son 
escaladas con el número de electrones activos de Toburen, 
...
{Sección eficaz de ionización molecular escalada por número de 
electrones débilmente ligados usando (a)~los números de Toburen 
$\nu_{\alpha}^T$, y (b) los números CDW $\nu_{\alpha}^{\text{CDW}}$ 
propuestos para las moléculas enlistadas en la Tabla~\ref{tab:families}. 
...
O debido al impacto de H$^+$ escaladas por el número de electrones 
...
por átomo, que han sido ajustados a partir de las secciones eficaces 
...
secciones eficaces moleculares escaladas por $n_e'$ se muestran en la 
...
ion incidente (reducida con $Z^\alpha$) en cualquier molécula (escalada 
con el número de electrones activos $n_e'$) con una dispersión de 
aproximadamente $\pm 20\%$. Si se consideran los valores experimentales 
...
incidente y escalada con el número de electrones activo $n_e$ del blanco 
...
las mediciones escaladas de Rudd~\textit{et al.}~\cite{Rudd:85,Rudd:83} 
...
Finalmente, para probar el rango de validez del SSM, se calcula la 
...
Para tomar en cuenta este efecto, se considera que el número total de 
...
En este Capítulo, se ha estudiado la ionización de blancos moleculares 
de interés biológico debido al impacto de iones de carga múltiple. La 
...
DIM), para modelar el proceso colisional. Se han calculado secciones 
...
Se estudiaron y calcularon valores medios de energía y ángulo de emisión 
...
colisión. Así, se define la primera regla de escala con el número de 
...
datos experimentales refuerza la ley de escala propuesta. Además, se 
probó la regla de escala CDW mediante la inclusión de datos 
...
energías intermedias a altas. La segunda regla de escala propuesta 
considera el ion incidente; la naturaleza del proyectil se reduce 
mediante el escaleo de la sección eficaz de ionización con la carga del 
ión, $Z^{\alpha}$, como una función de la energía incidente reducida 
$E/Z^{2-\alpha}$, siendo $\alpha=1.2$. La última regla propuesta combina 
el escaleo de la sección eficaz de ionización total con el número de 
electrones activos CDW y la reducción con la carga del ión, $Z^{\alpha}$. 
Esta combinación conduce a una ley de escala de tipo universal, 
independiente de la carga del ion y el blanco molecular. Las tres reglas 
de escala obtenidas mediante el modelo SSM--CDW para los 80 sistemas 
colisionales examinados fueron comparadas con datos experimentales 
disponibles. Finalmente, la generalidad de la regla de escala 
independiente fue puesta a prueba mediante la inspección de un número 
