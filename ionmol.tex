...
debido a su implementación en medicina~\cite{Baskar:12,Solov:09} y 
ciencias del ambiente~\cite{Gafur:18,FerrazDias:13}. El estudio del daño 
causado por proyectiles cargados en blancos biológicos es relevante 
debido a su aplicación en terapias contra el cáncer. 
La ionización de moléculas biológicas por haces de iones de carga 
múltiple constituye el principal mecanismo de daño celular. %Así, la 
%efectividad de la radiación depende de las características de los 
%proyectiles. En particular, estudios teóricos y experimentales con 
%diferentes proyectiles han concluido que los iones de carbono podrían 
%ser los más apropiados para dicha implementación~\cite{Mohamad:17}. 
...
Miraglia:09}. 
Esta aproximación constituye una de las teorías más exitosas 
para calcular procesos de ionización a energías intermedias y 
altas~\cite{Miraglia:08,Miraglia:09,Montanari:17-iongasesnobles}. 
Esta metodología también fue utilizada para modelar 
...
Recientemente, L\"udde \textit{et al.}~\cite{Ludde:16,Ludde:18,
...
apantallamiento. En este caso, las secciones eficaces atómicas se 
obtienen a partir de la teoría del functional densidad dependiente del 
tiempo. 

En este capítulo se estudia la ionización de moléculas complejas de 
interés biológico por impacto de iones de carga múltiple. El proceso 
colisional se describe a partir del método CDW-EIS~\footnote{Por 
simplicidad, la aproximación CDW-EIS será referida como CDW}, mientras 
que la complejidad del blanco es modelado implementando el modelo 
estequiométrico simple (\acs{ssm}). 
Además, se estudian diversas leyes de escala que permiten expresar las
secciones eficaces de ionización molecular de forma independiente del 
...
El modelo que se implementa en este trabajo para describir la ionización 
de sistemas moleculares asume que las moléculas están compuestas por 
átomos aislados e independientes. Por ello, primeramente se examina la
ionización de los sistemas atómicos que componen los blancos biológicos 
de interés. 
La sección eficaz de ionización $\sigma_{\alpha}$ del átomo $\alpha$ se 
obtiene a partir de la aproximación del método CDW en combinación con el 
método de inversión depurada (DIM). Los blancos atómicos se describen a 
partir del DIM, detallado en el Capítulo~\ref{chap:iondim}. Luego, las 
funciones de onda radiales de los estados inicial (ligado) y final 
(continuo) se obtienen resolviendo las correspondientes ecuaciones de 
Schr\"odinger definidas por estos potenciales. Particularmente, los 
estados del blanco, antes y después de la colisión, se obtienen 
implementando en el código~\textsc{radialf}, desarrollado por Salvat y 
colaboradores~\cite{salvat1995}.
...
10~MeV/amu. Para los proyectiles con cargas más altas, 
el valor de energía mínimo donde se espera que la aproximación tenga 
validez aumenta hasta 400~keV. Los valores teóricos CDW se comparan con 
secciones eficaces experimentales disponibles en la 
...
examinan las distribuciones energéticas y angulares medias de los 
electrones ejectados de blancos atómicos constituyentes según el método 
CDW.
...
(publicados recientemente en la Ref.~\cite{Sens:20}), la comparación 
...
experimental disponible~\cite{Bhattacharjee:19} en un factor 2. Esta 
...
superiores a 300~eV. Si bien las mediciones de ionización por impacto de 
electrones están por encima de los resultados SSM--CDW para todos los 
objetivos moleculares, estos concuerdan muy bien con otras predicciones 
teóricas de ionización por impacto de 
electrones~\cite{mozejko2003,tan2018}. Más aún, cálculos recientes de
Zein y colaboradores~\cite{Zein:21}, que emplean el modelo de encuentro 
...
teóricas difieren en un factor 2 de las mediciones experimentales de 
...
clásicas Monte Carlo, tienen amplitudes mayores a estos valores 
...
protones son significativamente más bajas que la teoría. Para la 
la molécula de THF, se encuentran disponibles una mayor cantidad de 
resultados experimentales de ionización por impacto de 
H$^+$~\cite{wang2016} y de e$^-$~\cite{bug2017,wolf2019,fuss2009}. Los 
resultados que se obtienen de la combinación del SSM y las secciones 
eficaces atómicas CDW muestran un buen acuerdo general con esta data.

En el caso de la molécula de agua, las mediciones son más abundantes que 
para el resto de los blancos. Los resultados 
...
igual al total de electrones del átomo menos el número correspondiente a 
la capa K. 
...
cada átomo $\alpha$. Los números de Toburen de los blancos atómicos 
considerados en este trabajo están dados por 
\begin{equation}
\nu_{\alpha}^T=\left\{ 
\begin{array}{ll}
1, & \text{para H,} \\
...
divididas por el número de electrones activos de Toburen, 
dando lugar a las secciones eficaces reducidas $\sigma_{e}^T$, para los 
blancos moleculares dados en la Tabla~\ref{tab:families}. Los resultados 
de esta aproximación se muestran en la Fig.~\ref{fig:newscaling}(a) en 
función de la energía de impacto, para diferentes proyectiles de carga 
múltiple. La figura verifica que la regla de Toburen se cumple a altas 
energías. Sin embargo, dado que la banda de resultados es muy ancha, no 
se puede considerar a su desempeño como satisfactorio.
...
que estos son muy similares a las secciones eficaces del carbono. Estas 
...
electrones activos para N, que se obtiene a partir de los resultados 
teóricos CDW, es diferente del dado por la Ec.~(\ref{eq:neToburen}). La 
Fig.~\ref{fig:neCDW}(b) muestra la respuesta de los cálculos atómicos a 
este nuevo valor.

A partir del estudio sistemático de las predicciones CDW para los 
sistemas proyectil--átomo, se propone una mejora a la regla de 
...
números efectivos que se proponen se debe a la cantidad de electrones 
...
muestran una dependencia lineal con el número de electrones efectivos 
...
una buena concordancia general, desde moléculas pequeñas~\cite{Lynch:76,
...
independiente refuerza la ley de escala dada por los números efectivos 
de electrones activos.
...
los datos experimentales disponibles que se muestran en la 
...
dos. Los niveles de energía atómica por encima de esta línea pueden 
considerarse como los correspondientes a los electrones débilmente 
ligados 
...
los cuatro electrones dados por los números efectivos. En el caso de 
oxígeno, 
...
se obtienen a partir de los cálculos de estructura molecular descriptos 
